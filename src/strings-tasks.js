...
function formatTime(minutes, seconds) {
  ...
  return `${addZerro(minutes)}:${addZerro(seconds)}`;
}
...
function containsSubstring(str, substring) {
  if (!str.includes(substring)) return false;
  return true;
}
...
function isPalindrome(str) {
  ...
  str
    ...
    .split('')
    .forEach((e) => {
      if (!sings.includes(e)) str1 += e;
    });
  for (let i = 0; i <= Math.floor(str1.length / 2) - 1; i += 1) {
...
function extractNameFromTemplate(value) {
  ...
  res += ' ';
  for (let i = 0; i < value.split(' ')[2].length; i += 1) {
    if (!sings.includes(value.split(' ')[2][i])) res += value.split(' ')[2][i];
  }
...
function encodeToRot13(str) {
  ...
  for (let i = 0; i < str.length; i += 1) {
    if (!alphabet.includes(str[i])) {
      ...
      res += alphabetRot13[alphabet.indexOf(str[i])];
    }
  }
  return res;
}
